...
const flagArgs = dashIndex !== -1 ? rawArgs.slice(0, dashIndex) : rawArgs.filter(a => a.startsWith('-'));
const targetArgs = dashIndex !== -1 ? rawArgs.slice(dashIndex + 1) : rawArgs.filter(a => !a.startsWith('-'));
